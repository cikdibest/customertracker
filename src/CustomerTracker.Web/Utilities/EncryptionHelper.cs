...
namespace CustomerTracker.Web.Utilities
{
    ...
    public static class EncryptExtensions
    {
        ...
        {
            return NinjectWebCommon.GetKernel.Get<IEncrypt>().Encrypt(text);
        }

        public static string Decrypt(this string text)
        {
            return NinjectWebCommon.GetKernel.Get<IEncrypt>().Decrypt(text); 
        }
    }

    public class AesEncryption : IEncrypt
    {
        private byte[] _salt = Encoding.ASCII.GetBytes("6Sq7890*65-2");

        private string _secretKey = "f3gh7*-4f55-";
